...
import { cosmos, getSigningStrideClient } from '../../src/codegen';
import { useChain, waitUntil } from '../src';
import './setup.test';

describe('Governance tests for stride', () => {
  ...
  it('vote on proposal from genesis address', async () => {
    ...
    const genesisSigningClient = await getSigningStrideClient({
      rpcEndpoint: getRpcEndpoint(),
  ...
  it('verify vote', async () => {
    ...
    expect(vote.options[0].option).toEqual(cosmos.gov.v1beta1.VoteOption.VOTE_OPTION_YES);
  }, 10000);

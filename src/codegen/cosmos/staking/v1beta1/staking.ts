...
export interface ValidatorSDKType {
  ...
  total_validator_bond_shares: string;
  /** Total number of shares either tokenized or owned by a liquid staking provider */

  total_liquid_shares: string;
}
...
export const Params = {
  encode(message: Params, writer: _m0.Writer = _m0.Writer.create()): _m0.Writer {
    ...
    if (message.validatorBondFactor !== "") {
      writer.uint32(50).string(message.validatorBondFactor);
    }

    if (message.globalLiquidStakingCap !== "") {
      writer.uint32(58).string(message.globalLiquidStakingCap);
    }

    if (message.validatorLiquidStakingCap !== "") {
      writer.uint32(66).string(message.validatorLiquidStakingCap);
    }

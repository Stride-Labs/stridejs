...
const GenesisState = {
  ...
  decode(input, length) {
    ...
    while (reader.pos < end) {
      ...
      switch (tag >>> 3) {
        case 1:
          message.identifiedFees.push(import_fee.IdentifiedPacketFees.decode(reader, reader.uint32()));
          break;
        case 2:
  ...
  fromPartial(object) {
    const message = createBaseGenesisState();
    message.identifiedFees = object.identifiedFees?.map((e) => import_fee.IdentifiedPacketFees.fromPartial(e)) || [];
  ...
  fromAmino(object) {
    const message = createBaseGenesisState();
    message.identifiedFees = object.identified_fees?.map((e) => import_fee.IdentifiedPacketFees.fromAmino(e)) || [];
  ...
  toAmino(message) {
    ...
    if (message.identifiedFees) {
      obj.identified_fees = message.identifiedFees.map((e) => e ? import_fee.IdentifiedPacketFees.toAmino(e) : void 0);
    } else {
...
const ForwardRelayerAddress = {
  ...
  encode(message, writer = import_binary.BinaryWriter.create()) {
    ...
    if (message.packetId !== void 0) {
      import_channel.PacketId.encode(message.packetId, writer.uint32(18).fork()).ldelim();
    }
    return writer;
  },
  decode(input, length) {

...
class QueryClientImpl {
  ...
  checksums(request = {
    ...
    const data = import_query.QueryChecksumsRequest.encode(request).finish();
    const promise = this.rpc.request("ibc.lightclients.wasm.v1.Query", "Checksums", data);
    return promise.then((data2) => import_query.QueryChecksumsResponse.decode(new import_binary.BinaryReader(data2)));
  }
  code(request) {

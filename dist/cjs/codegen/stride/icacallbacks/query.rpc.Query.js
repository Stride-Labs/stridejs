...
class QueryClientImpl {
  ...
  callbackData(request) {
    const data = import_query.QueryGetCallbackDataRequest.encode(request).finish();
    const promise = this.rpc.request("stride.icacallbacks.Query", "CallbackData", data);
    return promise.then((data2) => import_query.QueryGetCallbackDataResponse.decode(new import_binary.BinaryReader(data2)));
  }
  callbackDataAll(request = {
    pagination: void 0
  }) {
    const data = import_query.QueryAllCallbackDataRequest.encode(request).finish();
    const promise = this.rpc.request("stride.icacallbacks.Query", "CallbackDataAll", data);
    return promise.then((data2) => import_query.QueryAllCallbackDataResponse.decode(new import_binary.BinaryReader(data2)));
  }
}

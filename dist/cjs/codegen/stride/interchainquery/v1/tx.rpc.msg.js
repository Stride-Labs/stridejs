...
class MsgClientImpl {
  ...
  submitQueryResponse(request) {
    const data = import_tx.MsgSubmitQueryResponse.encode(request).finish();
    const promise = this.rpc.request("stride.interchainquery.v1.Msg", "SubmitQueryResponse", data);
    return promise.then((data2) => import_tx.MsgSubmitQueryResponseResponse.decode(new import_binary.BinaryReader(data2)));
  }
}

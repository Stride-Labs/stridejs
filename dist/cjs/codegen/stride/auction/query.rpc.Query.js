...
class QueryClientImpl {
  ...
  auction(request) {
    const data = import_query.QueryAuctionRequest.encode(request).finish();
    const promise = this.rpc.request("stride.auction.Query", "Auction", data);
    return promise.then((data2) => import_query.QueryAuctionResponse.decode(new import_binary.BinaryReader(data2)));
  }
  auctions(request = {
    pagination: void 0
  }) {
    const data = import_query.QueryAuctionsRequest.encode(request).finish();
    const promise = this.rpc.request("stride.auction.Query", "Auctions", data);
    return promise.then((data2) => import_query.QueryAuctionsResponse.decode(new import_binary.BinaryReader(data2)));
  }
}

...
class QueryClientImpl {
  ...
  epochProvisions(request = {}) {
    const data = import_query.QueryEpochProvisionsRequest.encode(request).finish();
    const promise = this.rpc.request("stride.mint.v1beta1.Query", "EpochProvisions", data);
    return promise.then((data2) => import_query.QueryEpochProvisionsResponse.decode(new import_binary.BinaryReader(data2)));
  }
}

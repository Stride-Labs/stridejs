...
class MsgClientImpl {
  ...
  execLegacyContent(request) {
    const data = import_tx.MsgExecLegacyContent.encode(request).finish();
    const promise = this.rpc.request("cosmos.gov.v1.Msg", "ExecLegacyContent", data);
    return promise.then((data2) => import_tx.MsgExecLegacyContentResponse.decode(new import_binary.BinaryReader(data2)));
  }
  vote(request) {

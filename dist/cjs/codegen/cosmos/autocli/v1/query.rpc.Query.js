...
class QueryClientImpl {
  ...
  appOptions(request = {}) {
    const data = import_query.AppOptionsRequest.encode(request).finish();
    const promise = this.rpc.request("cosmos.autocli.v1.Query", "AppOptions", data);
    return promise.then((data2) => import_query.AppOptionsResponse.decode(new import_binary.BinaryReader(data2)));
  }
}

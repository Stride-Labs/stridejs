...
class QueryClientImpl {
  ...
  inflation(request = {}) {
    const data = import_query.QueryInflationRequest.encode(request).finish();
    const promise = this.rpc.request("cosmos.mint.v1beta1.Query", "Inflation", data);
    return promise.then((data2) => import_query.QueryInflationResponse.decode(new import_binary.BinaryReader(data2)));
  }
  annualProvisions(request = {}) {
    const data = import_query.QueryAnnualProvisionsRequest.encode(request).finish();
    const promise = this.rpc.request("cosmos.mint.v1beta1.Query", "AnnualProvisions", data);
    return promise.then((data2) => import_query.QueryAnnualProvisionsResponse.decode(new import_binary.BinaryReader(data2)));
  }
}
